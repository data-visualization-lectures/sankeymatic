...
class CloudApi {
    ...
    static async listProjects() {
        ...
        return this.request(`/api/projects?app=${APP_NAME}`);
    }
            ...
            listContainer.onclick = async (e) => {
                if (e.target.classList.contains('load-btn')) {
                    ...
                    try {
                        ...
                        // projectData might be the project record with 'data' field, or just the data depending on API.
                        // According to plan: "クライアントへ JSON をそのまま返す" if it downloads file. 
                        // BUT api spec says: 
                        // GET /api/projects/:id -> finds record, downloads from Storage, returns JSON.
                        // Assuming the returned JSON IS the content saved in 'data' field of saveProject request body.
                        // Wait, saveProject body was { name, app_name, data }.
                        // So storage probably contains { name, app_name, data }.
                        // OR storage contains just `data`. The spec says "Draft RawGraphs JSON". 
                        // For SankeyMATIC, we should save exactly what we need.

                        // Adjusting assumption: The returned object IS the saved data object.

                        // In saveProject above: we sent { name, app_name, data: currentData }
                        // Wait, standard Cloud Save logic usually saves the 'data' part as the file content.
                        // Let's assume the API returns exactly what `data` was.

                        // However, spec says:
                        // "Body: { name, app_name, data: {...} }"
                        // "Supabase Storage へ JSON データをアップロード（パス: {user_id}/{uuid}.json）"
                        // So if I send the whole body as JSON, the whole body is in the file.
                        // If I only send 'data' as the file content, then I get 'data'.
                        // The API description says "JSONデータをアップロード". It's ambiguous if it unwraps 'data'.
                        // Usually, these APIs dump the request body 'data' property into the file.
                        // Let's assume `projectData` IS the Diagram Data we saved.

                        // Check if it's wrapped
                        let actualData = projectData;
                        if (projectData.data && !projectData.flows) {
                            // It might be wrapped in 'data' key if the API saves the whole payload
