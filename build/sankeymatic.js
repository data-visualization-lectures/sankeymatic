...
\n${outputSettings()}\n`;
  ...
  glob.saveDiagramAsJSON = () => {
    ...
    const settings = {};
    skmSettings.forEach((fldData, fldName) => {
      // Skip internal settings (mirroring the text export behavior)
    ...
    // 2. Gather Flows
    // Use removeAutoLines to get just the user's input text
    ...
    // 4. Construct Object
    const jsonObject = {
      credits: 'Made with SankeyMATIC',
      timestamp: glob.humanTimestamp(),
      flows: flows,
      settings: settings,
      moves: moves,
    };
    ...
    downloadATextFile(
      ...
      `sankeymatic_${glob.fileTimestamp()}.json`
    );
  };
